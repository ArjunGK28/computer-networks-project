...
class HopStats:
    ...
    def Building a traceroute and ping based network diagnostics tool provides deep insights into how real-world networks function. It demonstrates that the Internet operates as a hop-by-hop distributed system where packets traverse multiple routers rather than taking a direct path. By measuring round-trip time (RTT), the tool reveals that latency is dynamic and influenced by factors such as distance, congestion, and routing decisions. Observing packet loss highlights the inherently unreliable nature of networks and the need for higher-level protocols like TCP to ensure reliability. The implementation of TTL shows how packets are prevented from looping indefinitely while also enabling hop-by-hop analysis through traceroute. Using ICMP messages illustrates their critical role in network diagnostics rather than data transmission. The tool also exposes dynamic routing behavior, path asymmetry between forward and return routes, and the impact of firewalls or security policies when certain hops do not respond. Additionally, it helps measure key performance metrics such as latency, jitter, and packet loss, which are used in real-world network evaluation. From a systems perspective, it provides hands-on understanding of low-level networking concepts like raw sockets and packet handling. Overall, it reinforces that real-world networks are complex, adaptive, and often unpredictable, and equips the developer with practical skills to analyze and troubleshoot network issues effectively.loss_pct(self) -> float:
        if self.sent == 0:
            return 0.0
